...
class LanguageController extends Controller
{
    ...
    public function importJson(Request $request)
    {
        try {
            ...
            $myLang = Language::where(['id' => $request->my_lang])->firstOr(function () {
                throw new \Exception('No language found');
            });

            if ($request->lang_id == 'system_generate') {
                $jsonArray = $this->getKeys();
            } else {
            ...
            file_put_contents(
                resource_path('lang/') . $myLang->short_name . '.json',
                json_encode($jsonArray, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)
            );
    ...
    public function getKeys()
    {
        ...
        foreach ($langKeys as $langKey) {
            $assocArray[rtrim($langKey)] = rtrim($langKey);
        }
    ...
    private function getLangKeys($path)
    {
        ...
        preg_match_all("/(?:@lang|trans)\('(.*?)(?<!\\\\)'\)/", $finalcode, $keys);
